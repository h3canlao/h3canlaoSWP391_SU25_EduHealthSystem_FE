...
import { cancelVaccinationCampaign, completeVaccinationCampaign, getVaccinationCampaignDetail, startVaccinationCampaign } from "@/services/vaccinationCampaignApi";
...
const VaccinationCampaignDetail = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await getVaccinationCampaignDetail(id);
  ...
  const handleAction = async (type) => {
    ...
    try {
      if (type === "start") {
        await startVaccinationCampaign(id);
        message.success("Bắt đầu chiến dịch thành công!");
      } else if (type === "complete") {
        await completeVaccinationCampaign(id);
        message.success("Đã chuyển sang hoàn thành!");
      } else if (type === "cancel") {
        await cancelVaccinationCampaign(id);
  ...
  const scheduleColumns = [
    ...
    },
    {
      ...
      render: (status, record) => {
        ...
        return (
          <Badge status={opt.badge} text={record.scheduleStatusName || opt.label} />
        );
      }
    },
    {
  ...
  return (
    <Card
       title={<span>Chi tiết chiến dịch tiêm chủng</span>}
    ...
    >
      ...
      <div style={{ marginTop: 40 }}>
        <h3 style={{ fontWeight: 600 }}>Danh sách lịch tiêm trong chiến dịch</h3>
        <Table
          rowKey="id"
          ...
          size="middle"
          bordered
          locale={{ emptyText: "Chưa có lịch tiêm nào" }}
          style={{ marginTop: 12 }}
        />
      </div>
    </Card>
